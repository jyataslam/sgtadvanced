...
class Student{
	...
	*/
	update(field, value){
		if (this.data[field] != this.data.id && this.data[field] != this.data.name && this.data[field] != this.data.course && this.data[field] != this.data.grade){
			return false
		} else {
			if (this.data[field] === this.data.name){
				this.data.name = value;
				this.domElements.name.text(this.data.name);
			} else if (this.data[field] === this.data.course){
				this.data.course = value;
				this.domElements.course.text(this.data.course);
			} else if (this.data[field] === this.data.grade){
				this.data.grade = parseInt(value);
				this.domElements.grade.text(this.data.grade);
			} else if (this.data[field] === this.data.id){
				this.data.id = value;
			}
			return true
		}
	...
	*/
	render(){
		...
		this.domElements.deleteButton = $('<button>').text('delete').on('click', this.handleDelete);
